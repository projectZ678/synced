...
local clone_char = function(model)
    ...
    -- FIX: Convert new physics constraints back to classic Motor6Ds for the clone
    for _, descendant in ipairs(new_clone:GetDescendants()) do
        ...
        elseif descendant:IsA("BallSocketConstraint") or descendant:IsA("HingeConstraint") then
            -- Destroy the physical ragdoll joints so the clone is purely animated by the Motor6D
            descendant:Destroy()
        end
    end
...
API.stop_animation = function()
    ...
    if clone_char then
        -- Original unchanged unanimating logic
...
API.play_animation = function(url, speed)
    ...
    -- Original Motor6D joint caching loop
