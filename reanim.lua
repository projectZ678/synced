...
local get_game_ragdoll_info = function(enable)
	...
	-- 1. Try strict PlaceId matching first
	if place_id == 15546218972 or place_id == 6884319169 then
		-- Mic Up and Mic Up 18+
		local remote = stalkie.services.replicated:FindFirstChild("event_rag")
		if remote then return remote, {"Ball"}, false end
	elseif place_id == 5991163185 then
		-- Spray Paint
		local remotes = stalkie.services.replicated:FindFirstChild("Remotes")
		if remotes and remotes:FindFirstChild("Physics") and remotes.Physics:FindFirstChild("Ragdoll") then
			return remotes.Physics.Ragdoll, {}, false;
		end
	elseif place_id == 5683833663 then
		-- Ragdoll Engine (uses LocalEvent, not RemoteEvent)
		local local_event = stalkie.services.replicated:FindFirstChild("LocalRagdollEvent")
		if local_event then return local_event, {enable}, true; end
	end;

	-- 2. Dynamic scanning fallback for hinge/ragdoll support
	for _, v in ipairs(stalkie.services.replicated:GetDescendants()) do
		if v:IsA("RemoteEvent") then
			local name = v.Name:lower()
			if name == "event_rag" then
				return v, {"Ball"}, false
			elseif name == "ragdoll" or name == "toggleragdoll" or name == "ragdollremote" then
				return v, {enable}, false
			end
		elseif v:IsA("BindableEvent") then
			if v.Name == "LocalRagdollEvent" then
...
local clone_char = function(model)
	...
	-- FIX: Convert new physics constraints (including Hinge) back to classic Motor6Ds for the clone
	for _, descendant in ipairs(new_clone:GetDescendants()) do
		if descendant:IsA("AnimationConstraint") then
			local m = Instance.new("Motor6D")
			m.Name = descendant.Name
			m.Part0 = descendant.Part0
			m.Part1 = descendant.Part1
			m.C0 = descendant.C0
			m.C1 = descendant.C1
			m.Parent = descendant.Parent
			descendant:Destroy()
		elseif descendant:IsA("BallSocketConstraint") or descendant:IsA("HingeConstraint") then
			-- Hinge/BallSocket constraints are now handled by the Motor6D — destroy the physical ragdoll joints so the clone is purely animated by the Motor6D
			descendant:Destroy()
		end
	end
	return new_clone;
...
API.stop_animation = function()
	...
	if clone_char then
		-- Original unchanged unanimating logic for motor, orig_c0 in pairs(stalkie.animation.original_motor_c0s) do
...
API.reanimate = function(bool, remote, args)
	...
	if bool then
		...
		cloned_char:WaitForChild("Animate").Disabled = false; -- ensure disabled after reset
...
API.play_animation = function(url, speed)
	...
	local clone_char = API.get_clone(player);
	if not clone_char then return "Cannot play animation, clone character not found."; end
	...
	if not keyframe_data then
		...
		keyframe_data = data;
		if typeof(keyframe_data) ~= "table" then
			return "Animation Error: Script from URL did not return a table.";
		end;
	...
	-- Original Motor6D joint caching loop (unchanged)
	...
	stalkie.connections.animation_hb = stalkie.services.run_service.Heartbeat:Connect(function(deltaTime)
		...
		-- Original untouched animation application
