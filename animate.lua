...
API.stop_animation = function()
    ...
    if clone_char then
        ...
        -- Reset AnimationConstraints
        for _, motor in pairs(stalkie.animation.joints) do
            if motor and motor.Parent and motor:IsA("AnimationConstraint") then
...
API.play_animation = function(url, speed)
    ...
    for _, descendant in ipairs(clone_char:GetDescendants()) do
        if descendant:IsA("Motor6D") then
            ...
            -- Only cache C0 for actual Motor6Ds
	...
	stalkie.connections.animation_hb = stalkie.services.run_service.Heartbeat:Connect(function(deltaTime)
		...
		for partName, pose_cframe in pairs(current_frame.Data) do
            ...
            if motor then
                ...
                local lerped_pose = next_pose_cframe and pose_cframe:Lerp(next_pose_cframe, alpha) or pose_cframe;

                if motor:IsA("AnimationConstraint") then
                    -- AnimationConstraints are written to Transform
                    motor.Transform = lerped_pose;
                elseif anim.original_motor_c0s[motor] then
                    -- Motor6Ds fallback to the original C0 modification
                    motor.C0 = anim.original_motor_c0s[motor] * lerped_pose;
                end
            end
		end
	end);
